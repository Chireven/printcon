...
export class UploadSessionManager {
     ...
     */
    public async createSession(): Promise<UploadSession> {
        const sessionId = crypto.randomBytes(16).toString('hex');
        const tempDir = path.join(os.tmpdir(), 'printcon-uploads', sessionId);
        ...
        console.log(`[UploadSession] Created session: ${sessionId}`);
    ...
    public async saveChunk(
        ...
        if (fileMetadata.receivedChunks.has(chunkIndex)) {
            console.log(`[UploadSession] Chunk ${chunkIndex} for ${fileName} already exists (idempotent)`);
            return false;
        }
        ...
        console.log(`[UploadSession] Saved chunk ${chunkIndex} for ${fileName} (${fileMetadata.receivedChunks.size}/${fileMetadata.totalChunks})`);
    ...
    public async assembleFile(sessionId: string, fileName: string): Promise<string> {
        ...
        const fileDir = path.join(session.tempDir, fileName);
        const outputPath = path.join(session.tempDir, fileName.replace(/\./g, '_'));
        ...
        console.log(`[UploadSession] Assembled ${fileName} from ${fileMetadata.totalChunks} chunks`);
    ...
    public async finalizeSession(sessionId: string): Promise<string> {
        ...
        console.log(`[UploadSession] Finalized session: ${sessionId}`);
    ...
    public async cleanupSession(sessionId: string): Promise<void> {
        ...
        if (session) {
            ...
            try {
                ...
                console.log(`[UploadSession] Cleaned up temp directory: ${session.tempDir}`);
            } catch (error: any) {
                console.error(`[UploadSession] Failed to delete temp directory: ${error.message}`);
            }

            // Remove from sessions map
            this.sessions.delete(sessionId);
            console.log(`[UploadSession] Removed session: ${sessionId}`);
        }
    }
    ...
    private startCleanupTimer(): void {
        this.cleanupInterval = setInterval(() => {
            ...
            for (const sessionId of expiredSessions) {
                console.log(`[UploadSession] Auto-cleaning expired session: ${sessionId}`);
                this.cleanupSession(sessionId);
            }

            if (expiredSessions.length > 0) {
                console.log(`[UploadSession] Cleaned up ${expiredSessions.length} expired session(s)`);
            }
    ...
    public async shutdown(): Promise<void> {
        ...
        console.log('[UploadSession] Manager shutdown complete');
    }
}
