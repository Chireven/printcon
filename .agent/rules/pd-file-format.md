...
- **Extension**: `.pd`
- **Encoding**: Standard ZIP format, renamed from `.zip` to `.pd`
...
└── payload/               (REQUIRED - Driver files directory)
    ...
    └── [additional files] (Supporting files, configs, etc.)
```
...
**Critical**: The `entryPoint` must be a valid path relative to the package root. Example: `payload/hpcu250u.inf`
